...
/**
 * A word suggestion engine that uses affinity analysis on
 * its bag of words to predict the next word in a sequence.
 */
public class WordSuggester extends BagOfWords {
    ...
    /**
     * Creates an empty word suggester of {@code sequenceLen} order and the given suggestion confidence level.
     ...
     * If {@code defaultSuggestions} is null, then {@link #DEFAULT_BASE_SUGGESTIONS} is used.
     *
     * @param sequenceLen       the size of each n-gram
     * @param suggestConfidence the threshold for how correct a suggestion must be
    ...
    public WordSuggester(
        ...
        baseSuggestions = defaultSuggestions == null ? DEFAULT_BASE_SUGGESTIONS : List.copyOf(defaultSuggestions);
     ...
     * {@inheritDoc}
     * <p>
     * Each time this is called, the message count is incremented by one
     * and this word suggester is marked as untrained.
     */
    @Override
     ...
     * <p>
     * If more words are given than {@link #getOrder()}, then only the
     * needed words are taken from the end. If the number of suggestions is
     * fewer than the length of {@link #getBaseSuggestions()}, the results are padded.
     * For a possible suggestion to make it into this list, it must have a
